...
|  대만산   |   3/8인치 소켓렌치 세트   |           필수적인 소켓[^3]은 8, 10, 12, 14mm 이다.            |
...
1. 14mm 소켓을 사용하여 서스펜션 고정 너트를 분리한다. 재조립 시 토크는 규정값을 알 수 없지만 풀림토크 기준 2.0~2.5kgf·m였으므로 이 정도로 조립하면 된다.
1. 12mm 소켓을 사용하여 짐대를 분리한다. 이 역시 서스펜션 고정 토크와 동일하게 재조립하면 된다.
